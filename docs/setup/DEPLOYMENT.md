...
The repository includes a `services/grn-api/samconfig.template.toml` file that serves as a template for SAM CLI configuration. All build and deploy flags are configured in this template to avoid CLI argument conflicts.
...
**For local development**: Create your own `services/grn-api/samconfig.toml` (which is gitignored):
...
node scripts/deploy-and-configure.mjs

# Using a specific profile
node scripts/deploy-and-configure.mjs --profile sandbox

# Using a specific region
node scripts/deploy-and-configure.mjs --profile sandbox --region us-west-2

# Using a custom stack name
node scripts/deploy-and-configure.mjs --stack-name my-custom-stack
```
...
```bash
node scripts/deploy-and-configure.mjs --config-only --profile sandbox

# Or using the longer form:
node scripts/deploy-and-configure.mjs --skip-build --skip-deploy --profile sandbox
```
...
```bash
node scripts/deploy-and-configure.mjs --skip-build --profile sandbox
```

## Script Options

```
usage: node scripts/deploy-and-configure.mjs [-h] [--profile PROFILE] [--region REGION]
...
5. **Creates .env File** - Generates `apps/grn/.env` with the following variables:
...
```bash
node scripts/deploy-and-configure.mjs --stack-name my-new-stack
```
...
After deployment, manually copy the outputs from CloudFormation and update `apps/grn/.env`:
...
# Copy apps/grn/.env.example to apps/grn/.env
cp apps/grn/.env.example apps/grn/.env

# Edit apps/grn/.env with the output values
```
...
```bash
node scripts/deploy-and-configure.mjs --ci --region us-east-1 --stack-name community-garden-prod
```
