...
const clientScriptUrl =
  import.meta.env.VITE_CLOUDWATCH_RUM_CLIENT_URL?.trim() ||
...
export function initializeCloudWatchRum(): void {
  ...
  const appMonitorId = envValue(import.meta.env.VITE_CLOUDWATCH_RUM_APP_MONITOR_ID);
  const identityPoolId = envValue(import.meta.env.VITE_CLOUDWATCH_RUM_IDENTITY_POOL_ID);
  const guestRoleArn = envValue(import.meta.env.VITE_CLOUDWATCH_RUM_GUEST_ROLE_ARN);
  const region = envValue(import.meta.env.VITE_CLOUDWATCH_RUM_REGION);
  const releaseId = envValue(import.meta.env.VITE_CLOUDWATCH_RUM_RELEASE_ID);
