...
#[serde(rename_all = "camelCase")]
pub struct ProKpiResponse {
    pub window_days: i32,
    pub funnel: HashMap<String, i64>,
    pub conversion_rate: f64,
}

pub async fn track_pro_event(
    request: &Request,
    _correlation_id: &str,
    ...
    if let Err(feature_locked) =
        entitlements::require_entitlement(&client, user_id, "pro.analytics.read").await
    {
    ...
    client
        .execute(
            "
            insert into pro_analytics_events (user_id, event_name, event_source, metadata)
